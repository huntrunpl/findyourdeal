...
// ---- FYD: lang sync v2 (panel is source of truth) ----


async function getLangV3(chatId, user, telegramLangCode = "") {
  const uid = Number(user?.id || 0);

  // 1) DB jest źródłem prawdy: users.lang (fallback na users.language)
  if (uid) {
    try {
      const r = await dbQuery(
        `SELECT COALESCE(lang, language, '') AS lang
         FROM public.users
         WHERE id=$1
         LIMIT 1`,
        [uid]
  ...
  // 2) fallback: telegram language_code itp.
  const guess0 = normLang(telegramLangCode || user?.language_code || user?.language || "");
  const guess = isSupportedLang(guess0) ? guess0 : FYD_DEFAULT_LANG;

  // 3) best-effort: zapisujemy wybór do users.lang / users.language,
  // ale tylko gdy są puste
  if (uid) {
    try {
      if (await hasColumn("users", "lang")) {
        await dbQuery(
          `UPDATE public.users
           SET lang=$2, updated_at=NOW()
           WHERE id=$1 AND (lang IS NULL OR lang='') AND (lang IS NULL OR lang='')`,
          [uid, guess]
        );
      }
...
async function getLangFromUsers(chatId, user, telegramLangCode = "") {
  const uid = Number(user?.id || 0);

  // 1) users.lang / users.language – źródło prawdy, preferujemy lang
  if (uid) {
    try {
      const r = await dbQuery(
      ...
      const vLang = normLang(r?.rows?.[0]?.lang || "");
      const vLanguage = normLang(r?.rows?.[0]?.language || "");
      const v0 = vLang || vLanguage;
      if (v0 && isSupportedLang(v0)) {
        return v0;
      }
    } catch {}
  }

  // 2) fallback: chat_notifications.language (+ sync do users)
  try {
    ...
    const v2 = normLang(r2?.rows?.[0]?.language || "");
    if (v2 && isSupportedLang(v2)) {
      if (uid) {
        try {
          await dbQuery(
            `UPDATE public.users
             SET lang=$2, language=$2, updated_at=NOW()
             WHERE id=$1`,
            [uid, v2]
          );
        } catch {}
  ...
  // 3) fallback: language_code z Telegrama (+ zapis wszędzie)
  const guess0 = normLang(telegramLangCode || user?.language_code || user?.language || "");
  const guess = isSupportedLang(guess0) ? guess0 : FYD_DEFAULT_LANG;
...
// FYD: wrapper – wszystkie wywołania getLangV2 korzystają z getLangFromUsers
async function getLangV2(chatId, user, telegramLangCode = "") {
  return await fydResolveLang(chatId, user, telegramLangCode);
};

// ---- FYD: unified language resolver (panel <-> Telegram) ----
async function fydResolveLang(chatId, user, telegramLangCode = "") {
  const uid = Number(user?.id || 0);

  // 1) users.lang / users.language – źródło prawdy
  if (uid) {
    try {
      const r = await dbQuery(
        `SELECT COALESCE(NULLIF(lang,''), NULLIF(language,'')) AS lang
         FROM public.users
         WHERE id=$1
         LIMIT 1`,
        [uid]
      );
      const v0 = normLang(r?.rows?.[0]?.lang || "");
      if (v0 && isSupportedLang(v0)) return v0;
    } catch {}
  }

  // 2) fallback tylko do odczytu: language_code z Telegrama
  const guess0 = normLang(telegramLangCode || user?.language_code || user?.language || "");
  const guess = isSupportedLang(guess0) ? guess0 : FYD_DEFAULT_LANG;
  return guess;
}
