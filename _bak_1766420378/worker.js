...
async function getUserLimits(telegramUserId) {
  ...
  try {
    ...
    const out = {
      ...
      // UWAGA: limit total linków bierzemy z /me.limits (to już jest policzone z user_entitlements_v)
      links_limit_total: Number(lim?.max_total_links ?? lim?.max_active_links ?? 0) || 0,

      // (fallback / zgodność wstecz) – gdyby limits nie było
      links_limit: Number(f.links_limit || 0) || null,

      // na przyszłość (jeśli będziesz dzielił fast/normal)
...
async function getDailyLimitForUserId(notifyPool, userId) {
  try {
    ...
    for (const row of chatRows) {
      ...
      switch (planName) {
        case "trial":
          ...
          break;
        case "starter":
          // FYD_MAX_DAILY_DYNAMIC_V1
          // limit dzienny = 200 + addon_qty*100 (z aktywnej subskrypcji)
          const __uid =
            (typeof userId !== "undefined" ? userId :
              (typeof user !== "undefined" ? (user?.id ?? null) :
                (typeof u !== "undefined" ? (u?.id ?? null) :
                  (typeof link !== "undefined" ? (link?.user_id ?? link?.userId ?? null) : null)
                )
              )
            );

          maxDaily = await getDailyLimitForUserId(notifyPool, __uid);
...
async function loopOnce() {
  ...
  for (const [tgId, userLinks] of byUser.entries()) {
    ...
    if (cap > 0 && sorted.length > cap) {
      console.log(`[user ${tgId}] worker cap links_limit_total=${cap} active_links_seen=${sorted.length} -> processing=${toProcess.length}`);
    }
